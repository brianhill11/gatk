...
public final class FindBreakpointEvidenceSpark extends GATKSparkTool {
    ...
    protected void runTool( final JavaSparkContext ctx ) {
        ...
        final FastqWriter fastqWriter =
                new FastqWriter(locations.alignerIndexFile, maxFASTQSize, fastqDir, gfaDir);
        alignedAssemblyOrExcuseList.addAll(
                generateFastqs(ctx, qNamesMultiMap, allPrimaryLines, intervals.size(),
                                includeMappingLocation, fastqDir != null, fastqWriter));

        alignedAssemblyOrExcuseList.sort( ( result1, result2) ->
                Integer.compare(result1.getAssemblyId(), result2.getAssemblyId()) );
    ...
    @VisibleForTesting interface FASTQHandler
     ...
     */
    @VisibleForTesting static List<AlignedAssemblyOrExcuse> generateFastqs( final JavaSparkContext ctx,
                                                                            final HopscotchUniqueMultiMap<String, Integer, QNameAndInterval> qNamesMultiMap,
                                                                            final JavaRDD<GATKRead> reads,
                                                                            final int nIntervals,
                                                                            final boolean includeMappingLocation,
                                                                            final boolean dumpFASTQs,
                                                                            final FASTQHandler fastqHandler ) {
    ...
    private static final class FastqWriter implements FASTQHandler {
        private static final long serialVersionUID = 1L;
        ...
        FastqWriter( final String alignerIndexFile, final int maxFastqSize,
        ...
        public AlignedAssemblyOrExcuse apply( final Tuple2<Integer, List<FastqRead>> intervalAndReads ) {
            ...
            final AlignedAssemblyOrExcuse alignedAssemblyOrExcuse;
            if (fastqSize > maxFastqSize) {
                alignedAssemblyOrExcuse = new AlignedAssemblyOrExcuse(intervalAndReads._1(),
                ...
                if ( fastqDir != null ) {
                    ...
                    SVFastqUtils.writeFastqFile(fastqName, pipelineOptions, readsList.iterator());
                }
                ...
                try ( final BwaMemAligner aligner = new BwaMemAligner(BwaMemIndexSingleton.getInstance(alignerIndexFile)) ) {
                    ...
                    alignedAssemblyOrExcuse = new AlignedAssemblyOrExcuse(intervalAndReads._1(), assembly, alignments);
                }
            }
